...
<%if (msg.equals(UserStateEnum.USER_ADD_PASSWORD_REPASSWORD_ERROR
					.getValue())) {%>
	
 window.alert('<%=UserStateEnum.USER_ADD_PASSWORD_REPASSWORD_ERROR
						.getDesc()%>');
<%}%>
...
<%}%>
<%if (msg.equals(UserStateEnum.USER_USER_NAME_EXIST.getValue())) {%>
	
 window.alert('<%=UserStateEnum.USER_USER_NAME_EXIST.getDesc()%>');
	...
	function sendNote() {
		...
		$.ajax({
			...
			success : function(data) {
				...
				$("#validatecodemsg").show();
			 	$("#validatecodemsg").html("短信已發送");
				}
			},
			error : function() {
				alert("網絡異常請稍後再試");
			...
			<div class="zclf login logns">
				...
				<dl>
					...
					.getParameter("username")%>" />
						<label id="username2"
							style="color: red;margin-left: 20px;display: none;"></label>
						<div class="lf red laba">用户名要求不低于6个字符</div>
					</dd>
					<dd>
						<label> <small>*</small>密码</label> <input name="password"
					...
					<dd>
						<label> <small>*</small>电话</label> <input name="phone" type="text"
							class="inputh lf"
							value="<%=request.getParameter("phone") == null ? "" : request
					.getParameter("phone")%>" />
					...
					<dd class="hegas">
						<label> <small>*</small>手機號</label> <input id="phoneNumber"
							name="phonenumber" type="text" class="inputh inputs lf" value="" />
