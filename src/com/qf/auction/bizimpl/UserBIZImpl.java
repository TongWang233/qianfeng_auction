...
public class UserBIZImpl implements UserBIZ {
	...
	@Override
	public String userLogin(String username, String password, String sysCode,
			String userInput, HttpSession session) {
		if (userInput == null || userInput.equals("") || sysCode == null) {
			return UserLoginStateEnum.USER_LOGIN_STATE_VALIDATE_CODE_ERROR
					.getValue();
		}
		...
		if (username == null || username.equals("")) {
			return UserLoginStateEnum.USER_LOGIN_STATE_USER_NAME_FAIL
					.getValue();
		}
		if (password == null || password.equals("")) {
			return UserLoginStateEnum.USER_LOGIN_STATE_USER_PASSWORD_FAIL
					.getValue();
		}
		User user = null;
		// 调用DAO层 实例化 user
		...
		if (user == null) {
			return UserLoginStateEnum.USER_LOGIN_STATE_USER_OR_PASSWORD_FAIL
					.getValue();
		}
		// 如果代码能进入到这里说明 登录成功
	...
	public List<User> findUserListByPage(BigDecimal pageIndex,
			BigDecimal pageSize) {
		return userDAO.findUserListByPage(pageIndex.subtract(new BigDecimal(1))
				.multiply(pageSize), pageSize);
	}
	...
	@Override
	public String userRegister(User user, String repassword, int validateCode,
			String phoneNumber) {
		if (StringUtil.isEmpty(user.getPassWord())
				|| StringUtil.isEmpty(repassword)
				|| StringUtil.isEmpty(user.getUserName())
				|| StringUtil.isEmpty(user.getPhoneNumber())
		...
		// 超时 这里不需要实现  注册本身 不需要这个业务处理  和安全的相关 的操作 可以考虑 添加这个业务
		// 实现超时判断的思路 就是  把 当前手机号 最后 创建那条数据抓出来  然后  和当前系统时间做对比 
