...
public class NoteDAOImpl implements NoteDAO {
	...
	public int addNote(Note note) {
		...
		try {
			...
			preparedStatement = connection
					.prepareStatement("insert into note (phone,validatecode,createTime) values (?,?,?)");
	...
	public int findValidateCodeByPhoneNumber(String phonenumber) {
		...
		try {
			connection = JDBCUtil.getConnection();
			preparedStatement = connection
					.prepareStatement("select validatecode from note where phone = ? order by createtime desc limit 0,1");
