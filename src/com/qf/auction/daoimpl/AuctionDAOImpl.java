...
public class AuctionDAOImpl implements AuctionDAO {
	...
	@Override
	public List<Auction> findAuctionListByPage(BigDecimal beginPageNum,
			BigDecimal pageSize) {
	...
	public BigDecimal getAllCount() {
		...
		try {
			connection = JDBCUtil.getConnection();
			preparedStatement = connection
					.prepareStatement("select count(*) as auctiontotalcount from auction ");
			resultSet = preparedStatement.executeQuery();
			while (resultSet.next()) {
	...
	public int updateAuction(Auction auction) {
		...
		try {
			connection = JDBCUtil.getConnection();
			preparedStatement = connection
					.prepareStatement("update  auction set AUCTIONNAME =? , AUCTIONSTARTPRICE=?,AUCTIONUPSET=?,AUCTIONSTARTTIME=?,AUCTIONENDTIME=?,AUCTIONDESC=?,            AUCTIONPICPATH=?,UPDATETIME=?     where     auctionid  = ?");
			preparedStatement.setString(1, auction.getAuctionName());
			preparedStatement.setDouble(2, auction.getAuctionStartPrice());
			preparedStatement.setDouble(3, auction.getAuctionUpset());
			preparedStatement.setTimestamp(4, auction.getAuctionStartTime());
	...
	public int updateAuctionStateByAuctionId(int auctionId) {
		...
		try {
			connection = JDBCUtil.getConnection();
			preparedStatement = connection
					.prepareStatement("update auction set state = 1 where auctionid = ?");
			preparedStatement.setInt(1, auctionId);
			executeCount = preparedStatement.executeUpdate();
		} catch (Exception e) {
			e.printStackTrace();
...
//					.prepareStatement("select * from auction where state <> 0 ");
			...
			preparedStatement = connection
				.prepareStatement("select * from auction where state = 1 or state = 2 ");
